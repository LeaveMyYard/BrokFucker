...
class WebApp:
    ...
    exceptions_responses = {404: lambda error: send_file("src/404.html")}
...
class RestAPI:
    ...
    DatabaseHandler.run_verification_code_clearer(
        timedelta(hours=12), timedelta(days=7)
    )
    ...
    def process_exception(ex, error):
        if not request.path.startswith("/api/") and ex in WebApp.exceptions_responses:
            ...
            return make_response(
                jsonify(RestAPI.exceptions_responses[ex](error)),
