...
class DatabaseHandler:
    ...
    @staticmethod
    def run_verification_code_clearer(
        run_each: timedelta, duration_to_delete: timedelta
    ):
        database = DatabaseHandler()
        database.logger.debug("Starting verification codes clearer.")
        database.logger.debug(
            f"Will run each {run_each} and remove codes that exist more then {duration_to_delete}"
        )
    ...
    def generage_new_random_hash() -> str:
        return base64.b32encode(
            hashlib.sha256(
                ("random_words" + str(datetime.now())).encode("utf-8")
            ).digest()
        ).decode("utf-8")
    ...
    def is_moderator(self, email: str) -> bool:
        ...
        self.cursor.execute(f"SELECT `type` FROM Users WHERE `email` = '{email}'")
        res = self.cursor.fetchone()
    ...
    def create_email_confirmation_request(self, email: str, password: str):
        ...
        self.logger.debug(
            f"New email with confirmation code `{random_hash}` was sent to `{email}`"
        )
    ...
    def verify_email_confirmation(self, code: str) -> Union[str, None]:
        ...
        try:
            ...
            raise EmailValidationError(
                -1204,
                "No such verification code exists or it was already used.",
                on_failed,
            )
        ...
        if (
            ...
            raise EmailValidationError(
                -1203, "Email verification time has passed.", on_failed
            )
        ...
        self.logger.debug(
            f"New user `{email}` has successfuly confirmed his email and created an account"
        )
    ...
    def create_new_lot(
        ...
        self.cursor.execute(f"SELECT `id` FROM Lots WHERE `date` = '{date}'")
    ...
    def get_user_avatar_link(self, user):
        self.cursor.execute(f"SELECT `avatar` FROM Users WHERE `email` = '{user}'")
    ...
    def set_security_checked(self, lot_id, checked):
        ...
        self.conn.commit()
        self.logger.debug(
            f"`Security checked` flag on lot with id `{lot_id}` is set to `{checked}`"
        )
    ...
    def get_lot_creator(self, lot_id):
        self.cursor.execute(f"SELECT `user` FROM Lots WHERE `id` = '{lot_id}'")
    ...
    def get_lot_photos(self, lot_id):
        self.cursor.execute(f"SELECT `photos` FROM Lots WHERE `id` = '{lot_id}'")
    ...
    def subscribe_user_to_lot(
        self, user, lot_id, type: SubscriptionTypes, message
    ) -> bool:
    ...
    def get_user_subscriptions(self, user):
        ...
        return [
            ...
            for lot, confirmed, type, message in self.cursor.fetchall()
        ]

    def get_approved_subscriptions(self):
        self.cursor.execute(f"SELECT * FROM ConfirmedSubscriptions")
        return [
            {
                "id": id,
                "user": user,
    ...
    def get_unapproved_subscriptions(self):
        self.cursor.execute(f"SELECT * FROM UnconfirmedSubscriptions")
        return [
            {
                "id": id,
                "user": user,
