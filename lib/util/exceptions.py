...
class NoJsonError(IndexedException):
    def __init__(self):
        super().__init__(
            -1004,
            "This request requires json to be send, but there was none or it was corrupted.",
        )
...
class NotEnoughDataError(IndexedException):
    def __init__(self, sent: List[str], needed: List[str]):
        super().__init__(
            -1005,
            f"This request requires json to have next fields: {needed}, but got only: {sent}.",
        )
...
class UserHasNoPhoneNumber(IndexedException):
    def __init__(self, user):
        super().__init__(
            -1006,
            f"User {user} can not select recall by phone number option"
            " as he has no phone number assigned to his account.",
        )
